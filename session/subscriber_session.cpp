...
namespace webrtc
{
subscriber_session::subscriber_session(std::string session_id, std::string stream_id, std::string remote_sdp_offer, uint64_t created_at_milliseconds)
...
std::string_view subscriber_session::state_string() const { return session_state_to_string(state_); }
...
void subscriber_session::set_state(session_state state, uint64_t updated_at_milliseconds)
{
    state_ = state;
    updated_at_milliseconds_ = updated_at_milliseconds;
}

void subscriber_session::set_local_sdp_answer(std::string local_sdp_answer, uint64_t updated_at_milliseconds)
{
