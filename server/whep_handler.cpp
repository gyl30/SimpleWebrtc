...
http_response_ptr whep_handler::patch_session(http_request_t& request, std::string_view session_id)
{
    ...
    for (auto& candidate : *candidates)
    {
        ...
        if (remote_ice_candidate_already_known(*session, candidate))
        {
            ...
            WEBRTC_LOG_DEBUG("WHEP trickle ice duplicate candidate ignored stream={} session={} mid={} mline={} end={} candidate_size={}",
                             session->stream_id(),
                             session->session_id(),
                             sdp_mid,
                             sdp_mline_index,
    ...
    WEBRTC_LOG_INFO(
        "WHEP trickle ice patch accepted stream={} session={} candidates={} duplicates={} end_of_candidates={} candidate_bytes={} "
