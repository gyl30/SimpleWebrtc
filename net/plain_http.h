#ifndef SIMPLE_WEBRTC_NET_PLAIN_HTTP_H
#define SIMPLE_WEBRTC_NET_PLAIN_HTTP_H
...
{
   public:
    plain_http_session(std::string id, http_handler&& handler, boost::beast::tcp_stream&& stream, boost::beast::flat_buffer&& buffer)
        : id_(std::move(id)), handler_(std::move(handler)), stream_(std::move(stream)), buffer_(std::move(buffer))
    {
        WEBRTC_LOG_DEBUG("{} created", id_);
    }
    ~plain_http_session() { WEBRTC_LOG_DEBUG("{} destroyed", id_); }
